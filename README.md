...
英中实验：
   - T04 英中大模型(训练完成900k)：
      - eval loss 降不下去
         - train loss正常下降，但是eval loss仅初期下降，后续缓慢增加
         - 最终eval loss与train loss差距较大
      - eval BLEU（tensorboard）较低，仅有8.343
   - T05 英中小模型(训练完成300k):
      - 现象同大模型，eval BLEU（tensorboard）仅有6.757
   - 分析：
      - 怀疑英中数据处理方式或训练方式存在问题，需要debug

英德实验：
   - T03 英德大模型(训练完成900k)
      - eval loss 降不下去
      - eval BLEU（tensorboard）为26.65,低于预期的28+
   - T06-T10 数据量测试实验(训练完成300k)
     - eval loss 降不下去
     - eval BELU与数据量的关系为：
       - 100%数据： 24.19
       - 50% 数据:  21.36
       ...
       - 6.25%数据：7.167
   - 分析：
       - 从loss看，eval loss仅在100k个iter内下降，后续均缓慢上升，怀疑训练过程也有问题
       - 假设训练过程没有问题，则需要解决infer和eval的问题
 
TODO:
   - 解决infer和eval的问题
   - 整体代码流程分析
      - 尝试回答实验页面上的问题
      - 尝试解决eval loss下降不下去的问题
   - 优化workspace
      - 将所有参数放到json文件中(参数配置优先级：命令行 > json文件 > 默认参数)
      - 每个实验的训练数据，eval数据均放到对应的实验文件夹下(可软链)
